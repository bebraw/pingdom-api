...
function init(config) {
  ...
  var updateApi = updateResources.map(function(updateResource) {
    return [
      'update' + capitalizeFirstLetterAndRemovePeriod(setResource),
      template.bind(undefined, config, baseUrl, setResource, 'put')
    ];
  });

  return annozip.toObject(legacyApi.concat(getApi).concat(setApi).concat(updateApi));
