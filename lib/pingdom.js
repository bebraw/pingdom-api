...
function init(config) {
  ...
  var baseUrl = 'https://api.pingdom.com/api/2.1/';
  ...
  var updateResources = ['checks', 'contacts', 'reports.email', 'reports.public',
    'settings'];
  var removeResources = ['checks', 'contacts', 'reports.email', 'reports.public',
    'reports.shared'];
  ...
  var getApi = getResources.map(function(getResource) {
    ...
    return [
      'get' + capitalizeFirstLetterAndRemovePeriod(getResource),
      template.bind(undefined, config, baseUrl, getResource, 'get')
    ];
  });

  var setApi = setResources.map(function(setResource) {
    return [
      'set' + capitalizeFirstLetterAndRemovePeriod(setResource),
      template.bind(undefined, config, baseUrl, setResource, 'post')
    ];
  });

  var updateApi = updateResources.map(function(updateResource) {
    return [
      'update' + capitalizeFirstLetterAndRemovePeriod(updateResource),
      template.bind(undefined, config, baseUrl, updateResource, 'put')
    ];
  });

  var removeApi = removeResources.map(function(removeResource) {
    return [
      'remove' + capitalizeFirstLetterAndRemovePeriod(removeResource),
      template.bind(undefined, config, baseUrl, removeResource, 'del')
    ];
  });

  return annozip.toObject(
    legacyApi.concat(getApi).
      concat(setApi).
      concat(updateApi).
      concat(removeApi)
    );
...
function template(config, baseUrl, property, method, options, cb) {
  ...
  if (config.accountEmail) {
    options.headers['Account-Email'] = config.accountEmail;
  }
